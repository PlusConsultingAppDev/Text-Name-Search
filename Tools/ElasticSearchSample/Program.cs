...
namespace ElasticSearchSample
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            LoadData(elasticClient);
            Console.ReadLine();
        }

        public static bool CreateIndex(ElasticClient client)
        {
            var indexDescriptor = new CreateIndexDescriptor("documentindex").Mappings(x => x.Map<DocumentEntry>(m => m.AutoMap()));
            var result = client.CreateIndex("documentindex", x => indexDescriptor);
            return result.IsValid;
        }

        public static void LoadData(ElasticClient client)
        {
            IEnumerable<DocumentEntry> data = CreateData();
            foreach (var item in data)
            {
                client.Index(item, x => x.Index("documentindex"));
            }
        }

        public static List<DocumentEntry> CreateData()
        {
            ...
            return new List<DocumentEntry>{ item };
        }
    }
}
